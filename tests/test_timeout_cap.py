...
class TestTimeoutCapInSource:
    ...
    def test_source_contains_timeout_cap_constant(self):
        ...
        source_path = os.path.join(
            os.path.dirname(os.path.dirname(__file__)), "src/metasploit_mcp/server.py"
        )
        with open(source_path, "r") as f:
            source = f.read()

        assert (
            "MAX_TOOL_TIMEOUT_SECONDS = 120" in source
    ...
    def test_run_exploit_uses_timeout_cap(self):
        ...
        assert (
            "timeout_seconds > MAX_TOOL_TIMEOUT_SECONDS" in run_exploit_source
        ...
        assert (
            "inactivity_timeout_seconds > MAX_TOOL_TIMEOUT_SECONDS" in run_exploit_source
    ...
    def test_run_post_module_uses_timeout_cap(self):
        ...
        assert (
            "timeout_seconds > MAX_TOOL_TIMEOUT_SECONDS" in run_post_source
        ), "run_post_module should check if timeout_seconds exceeds MAX_TOOL_TIMEOUT_SECONDS"
        assert (
            "inactivity_timeout_seconds > MAX_TOOL_TIMEOUT_SECONDS" in run_post_source
    ...
    def test_run_auxiliary_module_uses_timeout_cap(self):
        ...
        assert (
            "timeout_seconds > MAX_TOOL_TIMEOUT_SECONDS" in run_aux_source
        ), "run_auxiliary_module should check if timeout_seconds exceeds MAX_TOOL_TIMEOUT_SECONDS"
        assert (
            "inactivity_timeout_seconds > MAX_TOOL_TIMEOUT_SECONDS" in run_aux_source
    ...
    def test_send_session_command_uses_timeout_cap(self):
        ...
        assert (
            "timeout_seconds > MAX_TOOL_TIMEOUT_SECONDS" in send_session_source
        ), "send_session_command should check if timeout_seconds exceeds MAX_TOOL_TIMEOUT_SECONDS"
        assert (
            "inactivity_timeout_seconds > MAX_TOOL_TIMEOUT_SECONDS" in send_session_source
