...
class TestExploitTestDataclass:
    ...
    def test_exploit_test_optional_fields(self):
        ...
        test = ExploitTest(
            name="Test", description="Desc", module="module", payload="payload", options={}
        )
...
class TestTestResultDataclass:
    ...
    def test_test_result_failure(self):
        ...
        result = TestResult(test_name="Test", success=False, error="Connection failed")
...
class TestMetasploitMCPClient:
    ...
    async def test_run_exploit(self, client):
        ...
        with patch.object(
            client, "call_tool", new=AsyncMock(return_value={"success": True})
        ) as mock_call:
    ...
    async def test_list_sessions(self, client):
        ...
        with patch.object(
            client, "call_tool", new=AsyncMock(return_value={"sessions": []})
        ) as mock_call:
    ...
    async def test_send_session_command(self, client):
        ...
        with patch.object(
            client, "call_tool", new=AsyncMock(return_value={"output": "uid=0"})
        ) as mock_call:
    ...
    async def test_start_listener(self, client):
        ...
        with patch.object(
            client, "call_tool", new=AsyncMock(return_value={"job_id": 1})
        ) as mock_call:
            await client.start_listener(
                payload_type="linux/x86/meterpreter/reverse_tcp", lhost="10.0.0.1", lport=4444
            )
...
class TestMetasploitable3TestHarness:
    ...
    def harness(self):
        ...
        return Metasploitable3TestHarness(
            target_ip="10.0.2.15", lhost="10.0.2.4", lport=4444, mcp_url="http://test-server:8085"
        )
    ...
    async def test_run_single_test_success(self, harness):
        ...
        with patch.object(
            harness.mcp_client,
            "run_exploit",
            new=AsyncMock(return_value={"success": True, "data": "Session 1 opened"}),
        ):
            with patch.object(
                harness.mcp_client,
                "list_sessions",
                new=AsyncMock(return_value={"success": True, "data": "Active sessions: 1"}),
            ):
                with patch.object(
                    harness.mcp_client,
                    "send_session_command",
                    new=AsyncMock(return_value={"success": True, "data": "uid=1000(testuser)"}),
                ):
    ...
    async def test_run_single_test_no_session(self, harness):
        ...
        with patch.object(
            harness.mcp_client,
            "run_exploit",
            new=AsyncMock(return_value={"success": True, "data": "Exploit completed"}),
        ):
            with patch.object(
                harness.mcp_client,
                "list_sessions",
                new=AsyncMock(return_value={"success": True, "data": "No active sessions"}),
            ):
                result = await harness.run_single_test(test)
    ...
    async def test_run_all_tests(self, harness):
        ...
        mock_result = TestResult(test_name="Test", success=True, session_id=1, duration_seconds=1.0)
    ...
    async def test_run_all_tests_stop_on_failure(self, harness):
        ...
        async def mock_run_test(test):
            ...
            if call_count == 1:
                ...
                # Second test fails
                return TestResult(
                    test_name=test.name, success=False, error="Failed", duration_seconds=1.0
                )
...
class TestSessionDetection:
    ...
    async def test_session_detection_various_formats(self):
        ...
        harness = Metasploitable3TestHarness(target_ip="10.0.0.1", lhost="10.0.0.1", lport=4444)
        ...
        for output, expected_session_id in test_outputs:
            with patch.object(
                harness.mcp_client,
                "run_exploit",
                new=AsyncMock(return_value={"success": True, "data": output}),
            ):
                with patch.object(
                    harness.mcp_client,
                    "list_sessions",
                    ...
                    with patch.object(
                        harness.mcp_client,
                        "send_session_command",
                        new=AsyncMock(return_value={"success": True, "data": "uid=0"}),
                    ):
                        result = await harness.run_single_test(test)

            assert (
                result.session_id == expected_session_id
            ), f"Failed to detect session ID in: {output}"
...
class TestIntegrationScenarios:
    ...
    async def test_proftpd_exploit_flow(self):
        ...
        harness = Metasploitable3TestHarness(target_ip="10.0.2.15", lhost="10.0.2.4", lport=4444)
