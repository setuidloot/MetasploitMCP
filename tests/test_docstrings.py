...
class TestDocstringContent:
    ...
    def test_run_exploit_warns_about_automatic_listener(self):
        ...
        assert "AUTOMATICALLY" in docstring.upper(), \
            "Docstring should emphasize automatic listener creation"
        assert "listener" in docstring.lower() or "handler" in docstring.lower(), \
            "Docstring should mention listeners/handlers"
        assert "DO NOT" in docstring.upper() or "DON'T" in docstring.upper(), \
    ...
    def test_run_exploit_has_usage_examples(self):
        ...
        assert "EXAMPLE" in docstring.upper() or "Example" in docstring, \
            "Docstring should include usage examples"
        assert "CORRECT" in docstring.upper() or "Correct" in docstring, \
            "Docstring should show correct usage"
        assert "INCORRECT" in docstring.upper() or "Incorrect" in docstring or "DON'T" in docstring.upper(), \
    ...
    def test_run_exploit_explains_when_to_use_start_listener(self):
        ...
        assert "start_listener" in docstring.lower(), \
            "Docstring should mention start_listener function"
        assert "when" in docstring.lower(), \
            "Docstring should explain when to use each function"
    ...
    def test_start_listener_warns_about_run_exploit_conflict(self):
        ...
        # Check for key warning phrases
        assert "run_exploit" in docstring.lower(), \
            "Docstring should mention run_exploit"
        assert "DO NOT" in docstring.upper() or "DON'T" in docstring.upper(), \
            "Docstring should have strong warning language"
        assert "conflict" in docstring.lower() or "port" in docstring.lower(), \
    ...
    def test_start_listener_lists_valid_use_cases(self):
        ...
        assert "USE" in docstring.upper() and "ONLY" in docstring.upper(), \
            "Docstring should specify when to use this function"
        assert "generate_payload" in docstring.lower(), \
    ...
    def test_start_listener_has_usage_examples(self):
        ...
        assert "CORRECT" in docstring.upper() or "Correct" in docstring, \
            "Docstring should show correct usage"
        assert "INCORRECT" in docstring.upper() or "Incorrect" in docstring, \
    ...
    def test_generate_payload_mentions_listener_requirement(self):
        ...
        assert "start_listener" in docstring.lower(), \
            "Docstring should mention start_listener"
        assert "listener" in docstring.lower() or "handler" in docstring.lower(), \
            "Docstring should mention listeners/handlers"
        assert "must" in docstring.lower() or "requirement" in docstring.lower(), \
    ...
    def test_generate_payload_has_workflow_guidance(self):
        ...
        assert "WORKFLOW" in docstring.upper() or "EXAMPLE" in docstring.upper(), \
            "Docstring should include workflow or example"
        assert "1" in docstring and "2" in docstring, \
            "Docstring should have numbered steps"
    ...
    def test_list_listeners_explains_purpose(self):
        ...
        assert "handlers" in docstring.lower(), \
            "Docstring should mention handlers"
        assert "jobs" in docstring.lower(), \
            "Docstring should mention jobs"
