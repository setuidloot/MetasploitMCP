...
class MetasploitMCPClient:
    ...
    async def call_tool(self, tool_name: str, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.debug(
            f"Calling tool: {tool_name} (actual: {actual_tool_name}) with args: {arguments}"
        )
        ...
        try:
            ...
            if not tool:
                ...
                raise Exception(
                    f"Tool '{actual_tool_name}' not found. Available: {available_tools[:5]}"
                )
    ...
    async def list_exploits(self, search_term: str = "", platform_filter: str = "") -> str:
        ...
        return await self.call_tool(
            "list_exploits", {"search_term": search_term, "platform_filter": platform_filter}
        )
    ...
    async def send_session_command(self, session_id: int, command: str) -> Dict[str, Any]:
        ...
        """
        return await self.call_tool(
            "send_session_command", {"session_id": session_id, "command": command}
        )

    async def start_listener(self, payload_type: str, lhost: str, lport: int) -> Dict[str, Any]:
        ...
        """
        return await self.call_tool(
            "start_listener", {"payload_type": payload_type, "lhost": lhost, "lport": lport}
        )
    ...
    async def terminate_session(self, session_id: int) -> Dict[str, Any]:
        ...
        Returns:
            Termination result
        """
        return await self.call_tool("terminate_session", {"session_id": session_id})
...
class Metasploitable3TestHarness:
        ...
        try:
            ...
            if isinstance(sessions_result, dict):
                ...
                # Check for status-based response
                    ...
                    logger.warning(
                        f"Failed to list sessions: {sessions_result.get('message', 'Unknown error')}"
                    )
                        ...
                        # Check in data field (as dict)
                            ...
                            if isinstance(data, str):
                                try:
                                    ...
                                    success = (
                                        "success" in data.lower() or "terminated" in data.lower()
                                    )
                                    error_msg = data
                        ...
                        # Check content field
                            ...
                            if isinstance(content, list) and len(content) > 0:
                                ...
                                if (
                                    ...
                                    success = (
                                        "success" in text.lower() or "terminated" in text.lower()
                                    )
                                    error_msg = text
    ...
    async def run_single_test(self, test: ExploitTest) -> TestResult:
        ...
        try:
            ...
            run_as_job = (
                self.run_as_job_override
                if self.run_as_job_override is not None
                else test.run_as_job
            )
            ...
            try:
                ...
                if isinstance(result_json, dict):
                    if "sessions" in result_json and result_json["sessions"]:
                        ...
                        "session_id_detected" in result_json and result_json["session_id_detected"]
                    ):
                        session_id = int(result_json["session_id_detected"])
                        logger.info(
                            f"Detected session ID from session_id_detected field: {session_id}"
                        )
                    ...
                    if sessions:
                        ...
                        for sid, sinfo in sessions.items():
                            if isinstance(sinfo, dict):
                                ...
                                if (
                                    test_module_lower in via_exploit
                                    and test_payload_lower in via_payload
                                ):
                                    session_id = int(sid)
                                    logger.info(
                                        f"✓ Found matching active session {session_id} created by {via_exploit} with {via_payload}"
                                    )
                                    break
                except Exception as e:
            ...
            if session_id:
                try:
                    ...
                    cmd_result = await self.mcp_client.send_session_command(
                        session_id=session_id, command="id"
                    )
            ...
            return TestResult(
                test_name=test.name, success=False, error=str(e), duration_seconds=duration
            )

    async def run_all_tests(
        self, continue_on_failure: bool = True, skip_cleanup: bool = False
    ) -> List[TestResult]:
...
Examples:
  ...
  python metasploitable3_test_harness.py --target 10.0.2.15 --lhost 10.0.2.4 --gateway --run-as-job
    ...
    parser.add_argument("--target", required=True, help="Target Metasploitable 3 IP address")
    parser.add_argument("--lhost", required=True, help="Local IP address for reverse connections")
    parser.add_argument(
        "--lport", type=int, default=4444, help="Local port for reverse connections (default: 4444)"
    )
    parser.add_argument(
        "--mcp-url",
    ...
    parser.add_argument(
        "--stop-on-failure", action="store_true", help="Stop testing after first failure"
    )
    ...
    try:
        ...
        if args.test:
            ...
            await harness.run_all_tests(
                continue_on_failure=not args.stop_on_failure, skip_cleanup=args.no_cleanup
            )
