...
class TestDescribeModule:
    ...
    async def test_describe_module_auxiliary(self):
        ...
        aux_options = {
            'RHOSTS': {'type': 'address', 'required': True, 'desc': 'Target hosts'}
        }
    ...
    async def test_describe_module_options_parsing(self, sample_module_info):
        ...
        test_options = {
            ...
            'ACTION': {'type': 'enum', 'required': False, 'enums': ['CHECK', 'EXPLOIT'], 'desc': 'Action to take'}
        }
...
""")
            ...
            eternalblue_doc.write_text("""# MS17-010 EternalBlue SMB Remote Windows Kernel Pool Corruption
...
class TestFindSimilarDocumentationFiles:
    ...
    def temp_docs_dir(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Create sample files
            (docs_path / 'exploit' / 'windows' / 'smb' / 'ms17_010_eternalblue.md').write_text('test')
    ...
    async def test_find_similar_respects_max_suggestions(self, temp_docs_dir):
        ...
        suggestions = await _find_similar_documentation_files(
            temp_docs_dir,
            'smb',  # Should match multiple files
            max_suggestions=2
        )
...
""")
    ...
    async def test_workflow_describe_then_get_docs(self, temp_docs_dir):
        ...
        module_options = {
            ...
            'SITEPATH': {'type': 'string', 'required': True, 'default': '/var/www', 'desc': 'Web path'},
        }
