...
def test_compatible_payloads_access_pattern():
    ...
    mock_module.payloads = ["windows/x64/meterpreter/reverse_tcp", "windows/x64/shell/reverse_tcp"]
...
def test_combined_filters():
    ...
    filtered = [
        ...
        if p.lower().startswith(platform + "/") or f"/{platform}/" in p.lower()
    ]
    ...
    print(
        f"✓ Combined filters work: platform='{platform}' + search='{search}' found {len(filtered)} payloads"
    )
