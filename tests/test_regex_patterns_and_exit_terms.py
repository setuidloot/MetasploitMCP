...
class TestRegexPatterns:
    ...
    def test_is_vulnerable_re_patterns(self):
        ...
        for text, should_match in test_cases:
            ...
            assert (
                result == should_match
            ), f"Pattern {'should' if should_match else 'should not'} match: '{text}'"
...
class TestRunCommandSafelyWithExitTerms:
    ...
    async def test_run_command_safely_with_exit_terms_session(self, mock_console):
        ...
        result = await run_command_safely(
            mock_console, "exploit", exit_terms_regexes=[SESSION_OPENED_RE]
        )
    ...
    async def test_run_command_safely_with_exit_terms_no_match(self, mock_console):
        ...
        def mock_read():
            ...
            call_count += 1
            if call_count == 1:
                return {"data": "Some regular output\n", "prompt": "", "busy": False}
            elif call_count <= 5:
                return {"data": "", "prompt": "", "busy": False}
            else:
        ...
        result = await run_command_safely(
            mock_console, "help", exit_terms_regexes=[IS_VULNERABLE_RE, IS_NOT_VULNERABLE_RE]
        )
    ...
    async def test_run_command_safely_with_exit_terms_multiple_patterns(self, mock_console):
        ...
        def mock_read():
            ...
            call_count += 1
            if call_count == 1:
                return {"data": "The target is not vulnerable\n", "prompt": "", "busy": False}
            elif call_count <= 5:
                return {"data": "", "prompt": "", "busy": False}
            else:
                return {
        ...
        result = await run_command_safely(
            mock_console, "check", exit_terms_regexes=[IS_VULNERABLE_RE, IS_NOT_VULNERABLE_RE]
        )
...
class TestExecuteModuleConsoleWithExitTerms:
    ...
    async def test_execute_module_console_without_exit_terms(self, mock_console):
        ...
        with patch("metasploit_mcp.server.get_msf_console") as mock_get_console:
            ...
            result = await _execute_module_console(
                module_type="auxiliary", module_name="test/module", module_options={}, command="run"
            )
...
class TestSessionDetectionWithRegex:
    ...
    def test_session_detection_with_regex(self):
        ...
        for output in session_outputs:
            ...
            ), f"Should extract session ID from matched text '{matched_text.decode('utf-8', errors='replace')}' for input: '{output}'"
