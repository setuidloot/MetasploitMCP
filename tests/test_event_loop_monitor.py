...
class TestConfigureEventLoopDebugging:
    ...
    def test_configure_debug_mode(self, event_loop):
        ...
        configure_event_loop_debugging(loop=event_loop, enable_debug=True, enable_watchdog=False)
    ...
    def test_configure_with_env_defaults(self, event_loop):
        ...
        with patch.dict(os.environ, {"ASYNCIO_DEBUG": "false", "EVENT_LOOP_WATCHDOG": "false"}):
...
class TestAsyncSessionCommandFixes:
    ...
    def test_shell_command_uses_asyncio_to_thread(self, metasploit_source):
        ...
        # Compare with whitespace removed so the assertion survives line-wrapping by formatters.
        compact_source = "".join(metasploit_source.split())
        assert (
...
class TestStopEventLoopMonitoring:
    ...
    def test_stop_cleans_up_watchdog(self):
        ...
        configure_event_loop_debugging(loop=loop, enable_debug=False, enable_watchdog=True)
