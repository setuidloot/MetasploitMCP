...
class TestGetKeepaliveManager:
    ...
    def test_returns_noop_when_ctx_is_none(self):
        ...
        manager = get_keepalive_manager(ctx=None, operation_name="Test operation")
    ...
    def test_returns_keepalive_manager_when_ctx_provided(self):
        ...
        mock_ctx = Mock()
        manager = get_keepalive_manager(ctx=mock_ctx, operation_name="Test operation")
    ...
    def test_passes_interval_parameter(self):
        ...
        mock_ctx = Mock()
        manager = get_keepalive_manager(ctx=mock_ctx, operation_name="Test operation", interval=5.0)
    ...
    def test_passes_progress_parameters(self):
        ...
        manager = get_keepalive_manager(
            ctx=mock_ctx, operation_name="Test operation", initial_progress=10, max_progress=80
        )
...
class TestToolsWithMockContext:
    ...
    @pytest.mark.asyncio
    async def test_list_exploits_reports_progress(
        self, mock_ctx, mock_msf_client, mock_asyncio_to_thread
    ):
    ...
    @pytest.mark.asyncio
    async def test_list_payloads_reports_progress(
        self, mock_ctx, mock_msf_client, mock_asyncio_to_thread
    ):
...
class TestKeepaliveManagerIntegration:
    ...
    async def test_manager_with_mock_ctx_start_stop(self):
        ...
        manager = get_keepalive_manager(
            ctx=mock_ctx, operation_name="Test operation", initial_progress=5, max_progress=90
        )
...
class TestFallbackKeepaliveManager:
    ...
    def test_fallback_class_has_required_methods(self):
        ...
        manager = manager_class(
            ctx=Mock(), interval=10.0, operation_name="Test", initial_progress=5, max_progress=90
        )
