...
class TestGetLocalIpAddresses:
    ...
    def test_get_local_ip_addresses_basic(self):
        ...
        with patch("socket.gethostname", return_value="test-host"):
            with patch(
                "socket.getaddrinfo",
                return_value=[(2, 1, 6, "", ("192.168.1.100", 0)), (2, 1, 6, "", ("127.0.0.1", 0))],
            ):
        ...
        with patch("socket.gethostname", return_value="test-host"):
            with patch(
                ...
                with patch("socket.socket") as mock_socket:
                    ...
                    assert (
                        count_192_168_1_100 == 1
                    ), f"Expected 1 occurrence of 192.168.1.100, got {count_192_168_1_100}"
...
class TestValidateBindAddress:
    ...
    async def test_validate_local_address(self):
        ...
        with patch(
            "metasploit_mcp.server.get_local_ip_addresses",
            return_value=["127.0.0.1", "192.168.1.100", "::1"],
        ):
...
class TestBindAddressIntegration:
    ...
    async def test_start_listener_valid_bind_address(self, mock_environment):
        ...
        with patch(
            "metasploit_mcp.server.get_local_ip_addresses",
            return_value=["127.0.0.1", "192.168.1.100"],
        ):
