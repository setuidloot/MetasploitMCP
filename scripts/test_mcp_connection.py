...
async def test_connection(mcp_url: str):
    ...
    try:
        ...
        print(
            f"  poetry run metasploit-mcp --transport http --host 127.0.0.1 --port {mcp_url.split(':')[-1].split('/')[0]}"
        )
        return False
...
async def test_tool_call(mcp_url: str):
    ...
    request_data = {
        ...
        "params": {
            "name": "list_exploits",
            "arguments": {"search_term": "proftpd", "platform_filter": ""},
        },
    }
...
async def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Test MCP connection to MetasploitMCP server")
    parser.add_argument(
        "--url",
        default="http://127.0.0.1:8085",
        help="MCP server URL (default: http://127.0.0.1:8085)",
    )
    parser.add_argument("--full", action="store_true", help="Run full test including tool calls")
