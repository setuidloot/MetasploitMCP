...
class EventLoopWatchdog:
    ...
    def start(self) -> None:
        ...
        self._thread = threading.Thread(
            target=self._watchdog_loop, name="EventLoopWatchdog", daemon=True
        )
        self._thread.start()
        logger.info(
    ...
    def _check_backlog(self) -> None:
        ...
        def count_tasks_callback() -> None:
            ...
            try:
                ...
                backlog_info.append(
                    {"pending_tasks": 0, "ready_callbacks": 0, "total_tasks": 0, "error": str(e)}
                )
            finally:
                response_event.set()
    ...
    def _log_backlog_warning(
        self, pending_tasks: int, ready_callbacks: int, total_tasks: int
    ) -> None:
                ...
                try:
                    ...
                    for task in pending[:10]:  # Limit to first 10
                        try:
                            ...
                            if coro:
                                coro_name = getattr(
                                    coro, "__qualname__", getattr(coro, "__name__", "unknown")
                                )
                    ...
                    task_details.append({"sample_tasks": task_info, "total_pending": len(pending)})
                except Exception as e:
...
def configure_event_loop_debugging(
    ...
    backlog_threshold = (
        backlog_threshold if backlog_threshold is not None else int(BACKLOG_THRESHOLD)
    )
...
def get_monitoring_stats() -> dict:
    ...
    stats = {
        "watchdog_running": _watchdog is not None
        and _watchdog._thread is not None
        ...
        async def wrapper(*args, **kwargs):
            ...
            try:
                ...
                if elapsed > threshold:
                    logger.warning(
                        f"⏱️ {func.__qualname__} took {elapsed:.3f}s " f"(threshold={threshold}s)"
                    )
    ...
    async def __aexit__(self, exc_type, exc_val, exc_tb) -> None:
        if self.start_time is not None:
            ...
            if elapsed > self.threshold:
                logger.warning(
                    f"⏱️ Block '{self.name}' took {elapsed:.3f}s " f"(threshold={self.threshold}s)"
                )
                if exc_type is None:
