...
class MetasploitInstanceManager:
    ...
    def __init__(
        ...
        logger.info(
            f"MetasploitInstanceManager initialized (base_port={base_port}, "
        ...
        async with self._lock:
            ...
            if agent_id in self.instances:
                ...
                if instance.is_healthy():
                    ...
                    logger.warning(
                        f"Existing instance for agent {agent_id} is unhealthy, recreating..."
                    )
    ...
    async def _wait_for_ready(self, instance: MetasploitInstance) -> MsfRpcClient:
        ...
        while (time.time() - start_time) < self.startup_timeout:
            ...
            if not instance.is_healthy():
                raise RuntimeError(
                    f"msfrpcd process died during startup for agent {instance.agent_id}"
                )
                ...
                async with self._lock:
                    for agent_id, instance in self.instances.items():
                        ...
                        if (current_time - instance.last_used) > self.inactivity_timeout:
                            logger.info(
                                f"Instance for agent '{agent_id}' has been inactive for "
                        ...
                        # Check if instance is unhealthy
                            ...
                            logger.warning(
                                f"Instance for agent '{agent_id}' is unhealthy, marking for termination"
                            )
                            agents_to_terminate.append(agent_id)
    ...
    def list_instances(self) -> Dict[str, Dict]:
        ...
        """
        return {agent_id: self.get_instance_info(agent_id) for agent_id in self.instances.keys()}
